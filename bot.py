...
async def help(ctx):
    embed = discord.Embed(title=ctx.author.name, description="You Got This!",
                          url="https://discord.gg/GfkjDChWbE", color=0x0abacd)
    embed.set_author(name="  All Commands", url="https://discord.gg/GfkjDChWbE",
                     icon_url="https://media.tenor.com/images/2d04da00ecf31f6065b436094f0b9c95/tenor.gif")
    embed.set_image(
        url="https://cdn.discordapp.com/attachments/782847710201774120/785742282833264640/Ant.gif")
    embed.add_field(name="$ping", value="Test Ping", inline=False)
    embed.add_field(name="$invite",
                    value="Jucy [Invite your friend]", inline=False)
    embed.add_field(name="$an [name]", value="or Random", inline=False)
    embed.add_field(name="$p ", value="ex: $p MandO", inline=False)
    embed.add_field(
        name="$dp", value="Someone DP(Display Picture)", inline=True)
...
async def ping(ctx):
    ...
    if ping > 100:
        ...
        embed.set_author(name=ctx.author.name,
                         url="https://discord.gg/GfkjDChWbE", icon_url=ctx.author.avatar_url)
        embed.set_thumbnail(
            url="https://cdn1.vectorstock.com/i/thumb-large/01/85/triangular-red-warning-hazard-symbol-vector-25180185.jpg")
        embed.set_footer(text="Copyright \u00a9 Rafi Rafi White-Ant")
        await ctx.channel.purge(limit=1)
        await ctx.send(embed=embed)
    else:
        embed = discord.Embed(
...
@client.command()  # Invite_dm
async def invite(ctx):
   await ctx.author.send("https://discord.gg/GfkjDChWbE")
   embed = discord.Embed(
       title=ctx.author.name, description="Invite Already Sent To Your DM 😉", color=0x01d9f1)
   embed.set_author(name="Team JUCY", url="https://discord.gg/GfkjDChWbE",
                    icon_url="https://cdn.discordapp.com/splashes/557864258617081858/053c45339b4d85c9cca13ffdc151d720.jpg?size=2048")
...
async def p(ctx, x):
    person = {
        ...
        "Mahib":  ["Skill : Graphic Designer", 0x6A1113, " Valorant , RDR ", "XI", "https://cdn.discordapp.com/attachments/782847710201774120/785788140249939978/1880726.png"],
    }
    embed = discord.Embed(
        title=f"Name: **{x}**  Class: **{person[x][3]}** ", description=f"Games: ** {person[x][2]} **", color=person[x][1])
    embed.set_author(name="Player introduction",
                     url="https://discord.gg/GfkjDChWbE")
    embed.set_image(url=person[x][4])
    embed.set_footer(text=f"{person[x][0]}")
    await ctx.channel.purge(limit=1)
    await ctx.send(embed=embed)
